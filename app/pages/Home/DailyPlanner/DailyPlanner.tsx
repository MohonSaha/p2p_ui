...
//   // Handle Add task: Show dialog for category selection
//   const handleAddTask = () => {
//     if (!newTask.trim()) return;
//     setIsDialogOpen(true);
//   };

//   // Handle category selection from dialog
//   const handleCategoryConfirm = async (categoryId: number) => {
//     try {
...
//                         removeTask(t.id);
//                       }}
//                       className="hidden md:block opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive"
//                       title="Delete task"
//                     >
...
const DailyPlanner = () => {
  ...
  const toggleTask = async (task: Task) => {
    try {
      ...
      const patchRes = await fetch(
        `${VITE_BASE_API}/daily-completion/${task.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ isDone: !task.isDone }),
  ...
  const handleCategoryConfirm = async (categoryId: number) => {
    try {
      setIsCreatingTask(true);
      const res = await fetch(VITE_BASE_API + "/daily-completion/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          task: newTask.trim(),
